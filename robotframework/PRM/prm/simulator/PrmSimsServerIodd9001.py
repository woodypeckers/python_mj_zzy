...
class PostHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        elif activityCode == u'020008': #组织机构代码查重接口
            return_msg = judgeOrgRepeat(reqDateTime, reqTransID)
...
def judgeOrgRepeat(reqDateTime, reqTransID):
    '''组织机构代码查重,不判断组织机构代码是否重复，直接返回000000'''
    temp = get_sys_time()
    recDateTime = temp
    rcvTransID = '999' + temp
    change_node_text('020008response.xml', reqDateTime, '/Msg/Header/ReqDateTime')
    change_node_text('020008response.xml', reqTransID, '/Msg/Header/ReqTransID')
    change_node_text('020008response.xml', recDateTime, '/Msg/Header/RcvDateTime')
    change_node_text('020008response.xml', rcvTransID, '/Msg/Header/RcvTransID')
    
    return_msg = read_xml('020008response.xml')
    return return_msg 
...
def exec_sql( sql, connstr = 'prm_auto/prm_auto_1000@10.12.3.197:1521/ora11g' ):
    '''修改成兼容模式
    mode1: rf/rf@emms227
